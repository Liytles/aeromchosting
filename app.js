document.addEventListener("DOMContentLoaded", () => {
    ...
    // YENİ: İşletim sistemine göre dosya ataması eklendi (.exe, .dmg, .deb)
    function detectOS() {
        let userAgent = window.navigator.userAgent;
        if (userAgent.indexOf("Windows") !== -1) return { name: "Windows", icon: "🪟", file: "AeroMC-Setup.exe" };
        if (userAgent.indexOf("Mac") !== -1) return { name: "macOS", icon: "🍎", file: "AeroMC-Installer.dmg" };
        if (userAgent.indexOf("Linux") !== -1) return { name: "Linux", icon: "🐧", file: "AeroMC-Linux.deb" };
        return { name: "Evrensel", icon: "📦", file: "AeroMC-Universal.zip" };
    }
    ...
    if(container && btn) {
        ...
        btn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                // YENİ: Animasyon parladığı anda doğru dosyayı otomatik indir
                const downloadLink = document.createElement('a');
                downloadLink.href = currentOS.file; 
                downloadLink.download = currentOS.file; 
